...
#_(defn app [{:keys [session biff/db] :as ctx}]
...
(defn echo [{:keys [body-params] :as ctx}]
  {:status 200
   :headers {"accept" "application/transit+json"
             "content-type" "application/transit+json"}
   :body body-params})

(defn pathom-endpoint [{:keys [body-params] :as ctx}]
  (let [pathom (p.eql/boundary-interface ctx)
        result (pathom body-params)]
    {:status 200
     :headers {"content-type" "application/transit+json"}
     :body result}))

(defresolver chef-id->xt-id
  [{:keys [chef/id]}]
  {::pco/output [:xt/id]}
  {:xt/id id})

(defn fetch-current-menu [{:keys [biff/db]} id]
  (let [menu-ids (biff/q db
                         '{:find [?menu-id]
                           :in [?chef-id]
                           :where [[?menu-id :menu/chef ?chef-id]
                                   [?menu-id :menu/status :current]]}
                         id)]
    (if (< (count menu-ids) 2)
      (ffirst menu-ids)
      (throw (ex-info "Chef has more than one current menu" {:chef/id id})))))

(defn archive-menu [ctx id]
  (biff/submit-tx ctx
                  [{:db/op :update
                    :db/doc-type :menu
                    :xt/id id
...
(defn add-dish [ctx menu-id {:dish/keys [name description servings price]}]
  (let [id (random-uuid)]
    (biff/submit-tx ctx
                    [{:db/doc-type :dish
                      :db/op :create
                      ...
                      :dish/price price
                      :dish/currency "USD"
                      :dish/created-at :db/now
                      :dish/updated-at :db/now}])
    id))

(defmutation create-menu [{:menu/keys [chef started-at ended-at delivery-method cuisines dietary-restrictions dishes] :as ctx}]
  {::pco/op-name 'menu/create}
  (when-let [current-menu-id (fetch-current-menu ctx chef)]
    (archive-menu ctx current-menu-id))
  (let [id (random-uuid)]
    (biff/submit-tx ctx
                    [{:db/doc-type :menu
                      :xt/id id
                      :menu/chef chef
                      :menu/started-at started-at
                      :menu/ended-at ended-at
                      :menu/delivery-method delivery-method
                      :menu/cuisines cuisines
                      :menu/status :current
                      :menu/dietary-restrictions dietary-restrictions}])
    (doseq [dish dishes]
      (add-dish ctx id dish))
    id))

(defresolver menu-id->xt-id
  [{:keys [menu/id]}]
  {::pco/output [:xt/id]}
  {:xt/id id})

(defresolver menu-id->chef-id
  [{:keys [menu/id] :as ctx}]
  {::pco/output [:menu/chef]}
  (let [menu (biff/lookup (:biff/db ctx) :xt/id id)]
    {:menu/chef (:menu/chef menu)}))

(defresolver dish-id->xt-id
  [{:keys [dish/id]}]
  {::pco/output [:xt/id]}
  {:xt/id id})

(defresolver dish-id->menu-id
  [{:keys [dish/id] :as ctx}]
  {::pco/output [:dish/menu]}
  (let [dish (biff/lookup (:biff/db ctx) :xt/id id)]
    {:dish/menu (:dish/menu dish)}))
...
(def module
  {:static {"/about/" about-page}
   ...
   :api-routes ["/api"
                ["/v1"
                 ["/echo" {:post echo}]
                 ["/pathom" {:post pathom-endpoint}]]]
   :on-tx notify-clients
   :resolvers [chef-id->xt-id
               menu-id->xt-id
               menu-id->chef-id
               dish-id->xt-id
               dish-id->menu-id]
   :mutations [create-menu]})
